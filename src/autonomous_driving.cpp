    ...
    const std::string &task_id, const geometry_msgs::msg::PoseStamped & /*pose*/)
{
  ...
  while (true)
  {
    ...
    if (retry_counter >= MAX_DRIVE_RETRIES)
    {
      ...
      RCLCPP_ERROR(node_->get_logger(), "Localization task %s failed.", task_id.c_str());
      break;
    }
    ...
    case AutonomousDrivingTaskState::DRIVING:
      if (vehicle_motion_state_ == MotionState::STOPPED)
      {
                ...
                MAX_EGO_HALT_TIME)
        {
          ...
          autoware_bridge_util_->updateTaskStatus(task_id, "HALTED", "HALT: 60 seconds elapsed while stopped");
        }
      }
      else
      {
