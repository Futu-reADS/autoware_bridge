...
AutowareBridgeNode::AutowareBridgeNode(
  ...
  std::shared_ptr<LocalizationTask> localization_task,
  std::shared_ptr<SetGoalTask> route_planning_task, 
  std::shared_ptr<DrivingTask> autonomous_driving_task)
  ...
  is_task_running_(false)
{
  ...
  status_service_ = this->create_service<autoware_bridge::srv::GetTaskStatus>(
    "check_task_status", std::bind( &AutowareBridgeNode::handle_status_request, this, 
  ...
  const std_msgs::msg::String::SharedPtr msg)
{
  ...
  startTaskExecution( msg->task_id.data, dummy_pose_stamped, autonomous_driving_task_);
}
...
void AutowareBridgeNode::startTaskExecution( const std::string & requested_task_id, 
  const geometry_msgs::msg::PoseStamped & pose_stamped, std::shared_ptr<BaseTask> task)
{
  RCLCPP_INFO( this->get_logger(), "Start task_id: %s", requested_task_id.c_str());
  autoware_bridge_util_->updateTaskStatus(requested_task_id, autoware_bridge_util_->TaskRequestType::STATUS, "PENDING");
...
void AutowareBridgeNode::publishTaskResponse(const std::string & task_id)
{
  if(autoware_bridge_util_->isTaskActive(task_id)){
    ...
    autoware_bridge_msgs::msg::TaskStatusResponse task_response = createTaskStatusResponse(task_id, task_status.status, task_status.reason);
...
void AutowareBridgeNode::cancelTaskCallback(const std_msgs::msg::String::SharedPtr msg)
{
  ...
  if (autoware_bridge_util_->isTaskActive(requested_task_id)){
    if (active_task) {
      ...
      RCLCPP_ERROR(this->get_logger(), "task_id: %s cancel request failed as active task pointer is null.",requested_task_id.c_str());
    }
  }
  else{
...
{
  if(autoware_bridge_util_->isTaskActive(task_id)){
    TaskCancellationInfo task_cancellation_status = autoware_bridge_util_->getTaskStatus(task_id).cancel_info;
    autoware_bridge_msgs::msg::TaskStatusResponse cancel_response = createTaskStatusResponse(task_id, task_cancellation_status.status, task_cancellation_status.reason);
    cancel_response_publisher_->publish(cancel_response);
  }
  else{
    RCLCPP_WARN( this->get_logger(), "Requested task_id: %s is not the active one to cancel.", task_id.c_str());
  }
}

autoware_bridge_msgs::msg::TaskStatusResponse AutowareBridgeNode::createTaskStatusResponse(std::string & task_id, 
...
int main(int argc, char * argv[])
{
  ...
  auto route_planning_task = std::make_shared<RoutePlanning>(node, autoware_bridge_util, is_task_running);
  auto autonomous_driving_task = std::make_shared<AutonomousDriving>(node, autoware_bridge_util, is_task_running);
