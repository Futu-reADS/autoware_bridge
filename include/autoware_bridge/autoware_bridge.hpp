...
public:
  AutowareBridgeNode(
    ...
    std::shared_ptr<RoutePlanning> route_planning_task, std::shared_ptr<AutonomousDriving> autonomous_driving_task);
...
private:
  ...
  void startTaskExecution( const std::string & requested_task_id, 
    const geometry_msgs::msg::PoseStamped & pose_stamped, std::shared_ptr<BaseTask> task);
  void startThreadExecution( const std::string & requested_task_id, const geometry_msgs::msg::PoseStamped & pose_stamped);
